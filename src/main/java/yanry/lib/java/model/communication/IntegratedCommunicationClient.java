package yanry.lib.java.model.communication;
...
import yanry.lib.java.model.communication.entity.MemoryCommunicationCaches;
		...
		if (jsonArray.length() > 0) {
			...
			try {
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					if ((flag
							& FLAG_REQUEST_PERSIST) == FLAG_REQUEST_PERSIST) {
						persist = true;
					}
				}
			} finally {
				...
				if (persist) {
					persistCommunicationCache(((MemoryCommunicationCaches) getCache()).serialize(FLAG_REQUEST_PERSIST));
				}
	...
	public class TextRequest extends RequestPack {
		...
		protected void doSend() {
			if (jsonData.length() > 0) {
				...
				int flag = FLAG_REQUEST_NORMAL;
				if (retryOnConnected) {
					flag |= FLAG_REQUEST_KEEP_TILL_READY;
				}
				if (guaranteed) {
					flag |= FLAG_REQUEST_GUARANTEED;
				}
				sendRequest(getIdentifier(), jsonData, flag);
				if (persist) {
					persistCommunicationCache(
							((MemoryCommunicationCaches) getCache()).serialize(FLAG_REQUEST_PERSIST));
				}
			}
		}
	}
