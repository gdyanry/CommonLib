...
import yanry.lib.java.model.communication.interfaces.*;
...
public abstract class CommunicationHandler {
    ...
    private void sendRequest(boolean isNewRequest) {
        Set<Object> requestIds = new HashSet<>();
     ...
     *
     * @param requestIds 用来装载之前发送失败而且需要顺带发送的请求id。
    ...
    public boolean cancelRequest(Object requestId) {
        boolean suppress = receive(requestId, null, 0);
        // shut the lock
        locked = false;
        return suppress || cache.getRequesponseToSendCache().remove(requestId);
    }
    ...
    public void reloadPendingRequests(Set<Object> requestIds) {
        for (Object id : requestIds) {
            // this request won't be found in pending requests when it's a one night request.
    ...
    private JSONArray getRequesponses(Set<Object> requestIds) {
        ...
        while ((re = cache.getRequesponseToSendCache().pop()) != null) {
            ...
            if (cache.getPendingRequestCache().containsValue(re)) {
                Logger.getDefault().e("reject request: ", re);
