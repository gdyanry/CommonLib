...
public class EventBus {
	...
	public EventBus() {
		...
		method_interface = new HashMap<Method, Class<?>>();
		event_method = new HashMap<Class<?>, Method>();
		interface_subscriber = new HashMap<Class<?>, List<Object>>();
		persistEvents = new LinkedList<Object>();
	}
	...
	public void register(Object subscriber) throws Exception {
		for (Class<?> i : subscriber.getClass().getInterfaces()) {
			if (interfaces.contains(i)) {
				...
				if (subscribers == null) {
					subscribers = new LinkedList<Object>();
