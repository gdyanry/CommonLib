...
public class FileMonitor implements Runnable {
	...
	public void monitor(WatchItem item) throws IOException {
		if (item.p == null) {
			ConsoleUtil.error(String.format("%s does not exist!", item.f.getAbsoluteFile()));
			return;
		}
		ConsoleUtil.debug("monitor: " + item);
		...
		if (items == null) {
			...
			ConsoleUtil.debug(String.format("register %s %s", item.p, Arrays.toString(kinds)));
		} else {
	...
	private void reregister(Path p, Set<Kind<?>> kinds) throws IOException {
		...
		p.register(ws, ks);
		ConsoleUtil.debug(String.format("re-register %s %s", p, Arrays.toString(ks)));
	}
		...
		if (item.p != null) {
			...
			if (items != null) {
				...
				if (items.remove(item)) {
					ConsoleUtil.debug("un-monitor: " + item);
