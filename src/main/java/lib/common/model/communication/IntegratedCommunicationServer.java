...
public abstract class IntegratedCommunicationServer<U> {
	...
	private TimerObjectPool<CommunicationHandler> anonymCHs;
	...
	/**
	 * 
	 * @param binaryTimeoutMinute
	 *            timeout minute of binary gotten by
	 *            {@link #onReceiveBinary(Object, byte[])}.
	 * @param idleAnonymCommunicationHandlerKeepAliveSeconds
	 *            keep alive time of idle {@link CommunicationHandler} objects
	 *            for handling anonymous requests.
	 * @param timer
	 */
	public IntegratedCommunicationServer(int binaryTimeoutMinute, int idleAnonymCommunicationHandlerKeepAliveSeconds,
			Timer timer) {
		pom = new PendingOperationManager<JSONObject>();
		chCache = new HashMap<String, CommunicationHandler>();
		requestCache = new HashMap<String, JSONObject>();
		if (binaryTimeoutMinute > 0) {
			binaryCache = new TimerCache<byte[]>(binaryTimeoutMinute * 60, timer);
		}
		anonymCHs = new TimerObjectPool<CommunicationHandler>(idleAnonymCommunicationHandlerKeepAliveSeconds) {
			@Override
		...
		handlers = new HashMap<String, ServerTagHandler<U>>();
	}
	 ...
	 * 
	 * @param text
	 *            text format: [session_id, [[flag, timestamp, {tag: [data,
	 *            ...], ...}], ...]]
	 * @param extra
	 *            extra data, which is passed in from upstream section, for
	 ...
	 * @param requestTimestamps
	 * @param isNewRequest
	 *            whether this send-text behavior is triggered by a new request.
	 */
